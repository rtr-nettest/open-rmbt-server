...
This token format (subsequently referred to as the *v1* token) is **deprecated**. To allow a smooth migration, the Control Server appends a second, *v2* token to the v1 token, separated by the marker `_#v2#`, yielding `<v1-token>_#v2#<v2-token>`. The v2 token additionally binds the client's source IP address and uses HMAC-SHA-256 instead of HMAC-SHA-1. Legacy RMBT servers simply ignore the unknown `_#v2#…` suffix (it is not part of the `[A-Za-z0-9+/=]` HMAC alphabet, so their token parser stops at the `_`) and validate the v1 part as before, whereas updated RMBT servers validate the v2 part and may optionally reject tokens that lack it. For details cf. section [TOKEN](#token-value).
...
The token carries the values needed to authenticate the test session. Two token schemas exist: the deprecated *v1* schema and the current *v2* schema. To allow a smooth migration the Control Server emits both as a single combined string:

```
<v1-token>_#v2#<v2-token>
```

The `_#v2#` marker is unambiguous because neither `_` nor `#` occurs in the base64 alphabet `[A-Za-z0-9+/=]` used by the surrounding fields. A server that understands v2 validates the part after `#v2#` (ignoring the v1 prefix); a legacy server is unaware of the marker — its token parser stops at the underscore preceding it — and validates the v1 prefix as before. A pure v1 token (no `#v2#` marker) is still accepted unless the server is configured for v2-only operation.

The shared secret (one of possibly several, to support key rotation) is known only to the Control Server and the RMBT server. If no key yields a valid signature, or the token is syntactically invalid, the server immediately terminates the connection.

##### v1 token (deprecated)

Constructed as `<UUID>_<TIMESTAMP>_<HMAC>` (the angle brackets are not part of the token):
...
The v2 token (`open-rmbt-udp-ping` schema) is the base64 encoding of 16 bytes:

```
time(4, big-endian) ‖ HMAC-SHA256(key, time)[0..8] ‖ HMAC-SHA256(key, time ‖ ip16)[0..4]
```

- `time`: the low 32 bits of the Unix start timestamp (big-endian). The server reconstructs the full timestamp to the value nearest its current time, so it remains valid across the 2³² s wrap.
- the first HMAC (bytes 4–11, the *time hash*) authenticates the start time and selects the key.
- the second HMAC (bytes 12–15, the *IP hash*) binds the client's source IP address: `ip16` is that address as a 16-byte IPv4-mapped IPv6 address (`::ffff:a.b.c.d` for IPv4) in network byte order.

The v2 token thus authenticates both the start time **and** the connecting source IP. If the time hash matches a key but the IP hash does not match the address the connection actually came from (or that address is unavailable), the connection is terminated.

##### Acceptance window (both schemas)

The client is only accepted if the current time is in a defined window depending on the start time encoded in the token.
...
The current server configuration uses a window of 20 seconds early, 90 seconds late.

A server may be configured for **v2-only** operation, in which case a token without a `#v2#` part is rejected even if its v1 HMAC is valid. This is intended for the final stage of the migration, once all clients emit combined tokens.
